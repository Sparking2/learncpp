...
// 3) A function named minmax() that takes two integers as input and returns back to the caller the smaller and larger number in a std::pair. 
// A std::pair works identical to a std::tuple but stores exactly two elements.

std::pair<int,int> minmax(int value1, int value2);

//) A function named getIndexOfLargestValue() that takes an integer array (as a std::vector), and returns the index of the largest element in the array.
...
const std::string getElement(const std::vector<string>& array);